...
# ==============================================================================
# Bash Script to Deploy or Redeploy a .NET Site from GitHub
# for Multi-Site .NET Server with Kestrel + Nginx
#
# Author: Travus Gonzalez
# Date: 2025-08-25
#
# Usage:
#   sudo ./deploy-site.sh domain.com https://github.com/user/repo.git [--force]
#
# Description:
#   Deploys or updates a .NET site from a GitHub repo into /var/www/domain.com
#   Sets up systemd Kestrel service + Nginx reverse proxy.
#   Automatically assigns next available port >= 5000.
#   --force will reclone the repo and optionally assign a new port.
# ==============================================================================
...
# --- Args ---
if [ $# -lt 2 ]; then
  echo "Usage: $0 domain.com https://github.com/user/repo.git [--force]"
  exit 1
fi
...
clone_or_update_repo() {
  if [ "$FORCE" = true ]; then
    ...
    rm -rf "$SITE_DIR/src"
    mkdir -p "$SITE_DIR"
    git clone "$REPO_URL" "$SITE_DIR/src"
  else
